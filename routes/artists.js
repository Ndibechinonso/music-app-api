...
router.post('/', async function (req, res) {
  try {
    ...
    const artists = await axios.get(`https://api.deezer.com/user/${id}/artists?access_token=${token}`)
    if(artists.data.error){
        return res.status(400).json(artists.data.error)
       }

  const recommendedArtists = await axios.get(`https://api.deezer.com/user/${id}/recommendations/artists?access_token=${token}`)
